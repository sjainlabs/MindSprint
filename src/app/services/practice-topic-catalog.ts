...
function buildTopicDefinition(
  ...
  const topicIdPrefix =
    track === 'k12' ? `grade-${slugify(groupKey)}` : `kumon-${slugify(groupKey)}`;
  const topicId = `${topicIdPrefix}-${slugify(topicName)}`;
  const grade =
    track === 'k12' ? [getGradeLevel(groupKey as keyof typeof PRACTICE_TOPIC_MAP.mathTopics)] : [];
  ...
  return {
    ...
    validation: {
      ...
      hasSupportedQuestionTypes: questionTypes.every(
        (type) => topic.questionTypes.includes(type) || DEFAULT_QUESTION_TYPES.includes(type),
      ),
