...
import { type AdvancedQuestionType, type AiWorksheet, type AiWorksheetRequest } from './ai-worksheet.service';
...
export interface PracticeTopicDefinition extends TopicModel {
  track: PracticeTrack;
  groupKey: string;
  groupLabel: string;
...
function difficultyRangeForTopic(track: PracticeTrack, groupKey: string): { min: number; max: number } {
  if (track === 'k12') {
...
function resolveGeneratorKey(topicName: string): PracticeTopicGeneratorKey {
  ...
  if (/(geometry|shape|angle|triangle|circle|quadrilateral|perimeter|area|volume|surface area|measurement|grid|transformation|pythagorean|drawing|conversion)/.test(normalized)) {
  ...
  if (/(calculus|derivative|integral|continuity|differentiability|differential equation|limits)/.test(normalized)) {
    return 'calculus';
  }
  if (/(matrix|vector|complex|set|relation|permutation|combination|determinant|university-level math|advanced calculus|pre-calculus)/.test(normalized)) {
    return 'advanced-math';
  }
  if (/(equation|expression|algebra|integer|irrational|real numbers|rational numbers|number systems)/.test(normalized)) {
    return 'algebra';
  }
  if (/(addition|subtraction|multiplication|division|factors|multiples|prime|composite|word problems?)/.test(normalized)) {
    return 'operations';
  }
  return 'number-sense';
}
...
function buildTopicDefinition(
  ...
  const topicIdPrefix = track === 'k12' ? `grade-${slugify(groupKey)}` : `kumon-${slugify(groupKey)}`;
  const topicId = `${topicIdPrefix}-${slugify(topicName)}`;
  const grade = track === 'k12' ? [getGradeLevel(groupKey as keyof typeof PRACTICE_TOPIC_MAP.mathTopics)] : [];
...
const K12_TOPICS = Object.entries(PRACTICE_TOPIC_MAP.mathTopics).flatMap(([groupKey, topicNames], groupIndex) =>
...
const KUMON_TOPICS = Object.entries(PRACTICE_TOPIC_MAP.kumonLevels).flatMap(([groupKey, topicNames], groupIndex) =>
  topicNames.map((topicName, topicIndex) =>
    buildTopicDefinition(
      topicName,
      'kumon',
...
export const PRACTICE_TOPIC_GROUPS: PracticeTopicGroup[] = [
  ...Object.entries(PRACTICE_TOPIC_MAP.mathTopics).map(([groupKey]) => ({
    ...
    track: 'k12' as const,
    topics: PRACTICE_TOPIC_CATALOG.filter((topic) => topic.track === 'k12' && topic.groupKey === groupKey),
  })),
  ...Object.entries(PRACTICE_TOPIC_MAP.kumonLevels).map(([groupKey]) => ({
    ...
    topics: PRACTICE_TOPIC_CATALOG.filter((topic) => topic.track === 'kumon' && topic.groupKey === groupKey),
  })),
];
...
function generateNumberSenseQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  const step = index + 1;
  ...
  if (/comparison/i.test(topic.name)) {
    ...
    return createQuestion(topic, index, difficulty, `Which number is larger: ${left} or ${right}?`, `${left}`, type);
  }
...
function generateOperationsQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  ...
  if (/subtraction|difference/i.test(topic.name)) {
    return createQuestion(topic, index, difficulty, `Solve: ${left + right} - ${right}`, `${left}`, type);
  }
  if (/multiplication/i.test(topic.name)) {
    return createQuestion(topic, index, difficulty, `Solve: ${scale} × ${index + 4}`, `${scale * (index + 4)}`, type);
  }
  if (/division/i.test(topic.name)) {
    const quotient = index + 3;
    const divisor = Math.max(2, Math.min(12, scale));
    return createQuestion(topic, index, difficulty, `Solve: ${quotient * divisor} ÷ ${divisor}`, `${quotient}`, type);
  }
  if (/factor|prime|composite/i.test(topic.name)) {
    const number = 12 + index * 3;
    return createQuestion(topic, index, difficulty, `How many factors does ${number} have?`, `${[6, 4, 6, 4][index % 4]}`, type);
  }
  return createQuestion(topic, index, difficulty, `Solve: ${left} + ${right}`, `${left + right}`, type);
}

function generateFractionsQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
...
function generateDecimalsQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  const left = (index + 2) / 10 + Math.round(difficulty / 25);
  const right = (index + 3) / 10;
  if (/operations/i.test(topic.name)) {
    return createQuestion(topic, index, difficulty, `Solve: ${left.toFixed(1)} + ${right.toFixed(1)}`, formatNumber(left + right, 1), type);
  }
  return createQuestion(topic, index, difficulty, `Write ${index + 1}/${(index + 2) * 2} as a decimal to the nearest hundredth.`, formatNumber((index + 1) / ((index + 2) * 2), 2), type);
}

function generateRatiosQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  if (/percent/i.test(topic.name)) {
    const percent = 10 + index * 5;
    const total = 40 + difficulty + index * 5;
    return createQuestion(topic, index, difficulty, `What is ${percent}% of ${total}?`, formatNumber((percent / 100) * total, 1), type);
  }
...
function generateGeometryMeasurementQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  if (/volume|solid/i.test(topic.name)) {
    const length = index + 3;
    const width = index + 2;
    const height = 2 + Math.round(difficulty / 30);
    return createQuestion(topic, index, difficulty, `Find the volume of a rectangular prism with side lengths ${length}, ${width}, and ${height}.`, `${length * width * height}`, type);
  }
  if (/conversion|measurement/i.test(topic.name)) {
    const meters = index + 2;
    return createQuestion(topic, index, difficulty, `Convert ${meters} meter(s) to centimeters.`, `${meters * 100}`, type);
  }
  if (/angle|triangle|circle|quadrilateral|transformation/i.test(topic.name)) {
    const first = 40 + index * 5;
    const second = 55 + index * 5;
    return createQuestion(topic, index, difficulty, `A triangle has angles ${first}° and ${second}°. What is the third angle?`, `${180 - first - second}`, type);
  }
  const sideA = index + 4;
  const sideB = index + 3;
  if (/area/i.test(topic.name)) {
    return createQuestion(topic, index, difficulty, `Find the area of a rectangle with side lengths ${sideA} and ${sideB}.`, `${sideA * sideB}`, type);
  }
  return createQuestion(topic, index, difficulty, `Find the perimeter of a rectangle with side lengths ${sideA} and ${sideB}.`, `${2 * (sideA + sideB)}`, type);
}

function generateDataHandlingQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  if (/probability/i.test(topic.name)) {
    const favorable = index + 2;
    const total = favorable + 4;
    return createQuestion(topic, index, difficulty, `A bag has ${favorable} red counters and 4 blue counters. What is the probability of drawing red?`, `${favorable}/${total}`, type);
  }
  if (/graph|plot/i.test(topic.name)) {
    const count = 6 + index * 2;
    return createQuestion(topic, index, difficulty, `A bar graph shows ${count} books read in April and 3 fewer books in May. How many books were read in May?`, `${count - 3}`, type);
  }
  const numbers = [index + 2, index + 4, index + 6, index + 8];
  const mean = numbers.reduce((sum, value) => sum + value, 0) / numbers.length;
  return createQuestion(topic, index, difficulty, `Find the mean of ${numbers.join(', ')}.`, `${mean}`, type);
}

function generateAlgebraQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  if (/inequalit/i.test(topic.name)) {
    const answer = index + 3;
    return createQuestion(topic, index, difficulty, `Find a value of x that makes x > ${answer - 1}. Enter the least whole number.`, `${answer}`, type);
  }
  const coefficient = index + 2;
  const answer = index + 4;
  return createQuestion(topic, index, difficulty, `Solve for x: ${coefficient}x = ${coefficient * answer}`, `${answer}`, type);
}

function generateFunctionsCoordinateQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  if (/coordinate/i.test(topic.name)) {
    const x1 = index + 1;
    const x2 = x1 + 4;
    return createQuestion(topic, index, difficulty, `What is the distance between points (${x1}, 0) and (${x2}, 0)?`, `${x2 - x1}`, type);
  }
  const x = index + 2;
  return createQuestion(topic, index, difficulty, `If f(x) = 2x + 3, what is f(${x})?`, `${2 * x + 3}`, type);
}

function generatePolynomialsQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  if (/quadratic/i.test(topic.name)) {
    const answer = index + 2;
    return createQuestion(topic, index, difficulty, `Solve x² = ${answer * answer}. Give the positive solution.`, `${answer}`, type);
  }
  const x = index + 2;
  return createQuestion(topic, index, difficulty, `Evaluate x² + 2x + 1 when x = ${x}.`, `${x * x + 2 * x + 1}`, type);
}

function generateTrigonometryQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  ...
  return createQuestion(topic, index, difficulty, `What is sin ${angle}°?`, answerByAngle[angle], type);
}

function generateCalculusQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  if (/integral/i.test(topic.name)) {
    const exponent = index + 2;
    return createQuestion(topic, index, difficulty, `Find ∫ ${exponent}x dx. Write only the coefficient of x².`, `${exponent / 2}`.replace(/\.0$/, ''), type);
  }
  const coefficient = index + 2;
  const exponent = 2;
  return createQuestion(topic, index, difficulty, `Find the derivative of ${coefficient}x². Write only the coefficient of x.`, `${coefficient * exponent}`, type);
}

function generateAdvancedMathQuestion(topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) {
  if (/matrix/i.test(topic.name)) {
    return createQuestion(topic, index, difficulty, 'What is the determinant of [[2, 1], [1, 2]]?', '3', type);
  }
  if (/vector/i.test(topic.name)) {
    return createQuestion(topic, index, difficulty, 'Find the magnitude of vector ⟨3, 4⟩.', '5', type);
  }
  if (/complex/i.test(topic.name)) {
    return createQuestion(topic, index, difficulty, 'What is i²?', '-1', type);
  }
  if (/set/i.test(topic.name)) {
    return createQuestion(topic, index, difficulty, 'How many elements are in the set {2, 4, 6, 8}?', '4', type);
  }
  return createQuestion(topic, index, difficulty, `How many outcomes are there when choosing 2 items from 4 distinct items?`, '6', type);
}

const GENERATORS: Record<PracticeTopicGeneratorKey, (topic: PracticeTopicDefinition, index: number, difficulty: number, type: AdvancedQuestionType) => AiWorksheet['questions'][number]> = {
...
export function generatePracticeWorksheetPreview(payload: AiWorksheetRequest): AiWorksheet {
  const topic = findPracticeTopicById(payload.topic) ?? defaultTopic();
  const generator = GENERATORS[topic.generatorKey];
  const questionCount = clamp(payload.questionCount ?? 8, 1, 20);
  const difficulty = clamp(Math.round(payload.difficulty ?? topic.subtopics[0]?.difficulty.min ?? 50), 0, 100);
  const questionTypes = payload.questionTypes?.length ? payload.questionTypes : topic.questionTypes;
  ...
  return {
    ...
    questions: Array.from({ length: questionCount }, (_, index) => {
      ...
      return generator(topic, index, clamp(difficulty + (index % 3) * 3, 0, 100), type);
    }),
    validation: {
      allQuestionsHaveAnswers: true,
      hasSupportedQuestionTypes: questionTypes.every((type) => topic.questionTypes.includes(type) || DEFAULT_QUESTION_TYPES.includes(type)),
